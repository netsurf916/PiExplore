...
async function fetchDigits(start, count, token) {
  ...
  while (offset < count) {
    ...
    const chunk = Math.min(MAX_PER_REQUEST, count - offset);
    const requestStart = start + BigInt(offset);
    const url = `${API}?start=${requestStart}&numberOfDigits=${chunk}`;
...
async function render() {
  ...
  try {
    digits = await fetchDigits(start, count, token);
    ...
    statusEl.textContent = `${rowWidth.toLocaleString()} digits/row × ${rows.toLocaleString()} rows · ${digits.length.toLocaleString()} digits`;
